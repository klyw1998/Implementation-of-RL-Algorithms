...
class TRPO:
    ...
    def __init__(self, hidden_dim, state_space, action_space, lmbda, kl_constraint, alpha, critic_lr, gamma, device):
        ...
        self.critic = ValueNet(state_dim, hidden_dim, 1).to(device)
    ...
    def hessian_matrix_vector_product(self, states, action_dists_old, vector):
        # calculate the product of the hessian matrix and a vector
        action_dists_new = torch.distributions.Categorical(self.actor(states))
    ...
    def line_search(self, states, actions, advantage, log_probs_old, action_dists_old, max_vec):
        ...
        for i in range(15):
            ...
            action_dists_new = torch.distributions.Categorical(actor_new(states))
    ...
    def update(self, transition_dict):
        ...
        log_probs_old = torch.log(self.actor(states).gather(1, actions)).detach()
        action_dists_old = torch.distributions.Categorical(self.actor(states).detach())
